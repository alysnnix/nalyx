...
let
  ...
  # This module is imported by wsl, desktop and homelab. WSL and desktop hold
  # the plaintext for every folder; the homelab takes part in each one only as
  # an encrypted-at-rest peer.
  ...
  # The two hosts that hold plaintext and the folder password.
  trustedPeers = [
    "wsl"
    "desktop"
  ];
...
in
{
  options.modules.services.syncthing.encryptionPasswordFile = lib.mkOption {
    ...
    description = ''
      ...
      trusted peers (wsl, desktop) and used to offer every folder to the
  ...
  config = {
    services.syncthing = {
      ...
      settings = {
        ...
        # Single shared work folder. Plaintext and bidirectional on wsl and
        # desktop; encrypted at rest on the homelab.
        ...
        folders.wrk = {
          ...
          # deletions and wiped ~17.5 GB on the other trusted peer of the day
          # (since retired from the fleet).
          #
          ...
          # left to mistake for a deletion. Keeping it receiveonly would now
          # just let an older peer's copy overwrite work done here.
        ...
        # resumed on the desktop and vice versa. Only projects/ is synced (the
        ...
        #
        # Bidirectional between wsl and desktop (sendreceive on both), and
        # encrypted onto the homelab, which never holds the folder password.
        #
        ...
        # importer.
        #
        # Bidirectional since 2026-09-10, where it used to be one-way into a
        # third host that has since left the fleet. It is safe here
        # specifically because of how Claude Code names its files: every
        # transcript is `<session-uuid>.jsonl` and every agent record is
        # `agent-<id>.meta.json`, so a given file has exactly one author
        # machine and two peers never write the same path. That is what makes
        # last-writer-wins harmless for this folder and not for the two below
        # it.
        folders.claude = {
          id = "claude";
        ...
        # omp harness history: sessions/*.jsonl transcripts plus the SQLite
        # index/state (history.db, agent.db, models.db) and blobs/.
        #
        # WSL and desktop are both sendonly: with no receiveonly peer left in
        # the fleet, they each push their own history and never accept remote
        # changes, so this folder now only lands, encrypted, on the homelab as
        # a backup. It is not read back from there.
        #
        # Caveat behind staying one-way instead of sendreceive like `claude`:
        # the SQLite dbs are WAL and written live, so a mid-write sync can land
        # a torn db on the receiving side. This folder carries agent.db plus
        # its -wal and -shm sidecars, and Syncthing treats those three as
        # unrelated files, so it can deliver a database from one instant with
        # the write-ahead log from another. That is corruption, not a
        # conflict, and no `maxConflicts` setting addresses it. `AGENTS.md`
        # and `RULES.md` in here are Nix-generated too (see their
        # .nix-sha256 companions), so writing them back would fight
        # activation.
        folders.omp = {
          id = "omp";
          path = if isHomelab then "/data/sync/omp-enc" else "/home/${vars.user.name}/.omp/agent";
          type = if isHomelab then "receiveencrypted" else "sendonly";
          devices = trustedPeers ++ lib.optionals (!isHomelab) untrustedPeer;
          maxConflicts = 0;
        };

        # Herdr config plus session state, between WSL and the homelab, where it
        # lands encrypted at rest. Not defined on the desktop, which does not
        ...
        # replicated at all.
        #
        # WSL is the only plaintext peer now that the second one has left the
        # fleet. Kept as sendreceive rather than narrowed to sendonly, so that
        # adding a future second peer here needs only a device entry and not a
        # type change.
        folders.herdr = lib.mkIf (isWsl || isHomelab) {
          id = "herdr";
