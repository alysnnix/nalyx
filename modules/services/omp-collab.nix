...
#   tailscale serve (:<servePort> TLS) → nginx (127.0.0.1:18084)
...
let
  ...
  servePort = config.modules.services.ompCollab.servePort;
  ompCollab = pkgs.callPackage ../../packages/omp-collab { };
in
{
  options.modules.services.ompCollab.servePort = lib.mkOption {
    type = lib.types.port;
    default = 443;
    description = ''
      Port `tailscale serve` publishes omp-collab on, over the tailnet.
